...
import { EVENT_TYPE_PREF_OPTIONS } from '../lib/eventPreferences'
...
export default function Preferences() {
  ...
  const [eventTypes, setEventTypes] = useState<EventPreference[]>([])
  ...
  useEffect(() => {
    ...
    async function load() {
      const [profileResp, storeResp, eventResp] = await Promise.all([
        supabase
          .from('profiles')
  ...
  const toggleEventType = async (id: string, enabled: boolean) => {
    setEventTypes((prev) =>
  ...
  const eventPrefByType = Object.fromEntries(
    eventTypes.map((e) => [e.event_type, e]),
  )
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="rounded-xl bg-surface border border-border p-6 shadow-sm">
        <div className="flex items-center justify-between">
          <div>
            <h3 className="text-sm font-semibold text-text">Feature Announcements</h3>
            <p className="text-sm text-text-muted mt-0.5">
      ...
      <div className="rounded-xl bg-surface border border-border p-6 shadow-sm">
        <h3 className="text-sm font-semibold text-text mb-2">Alert Types</h3>
        <p className="text-sm text-text-muted mb-4">
          Choose which kinds of stock alerts you want by email. Watched items still follow these settings.
        </p>
        <div className="divide-y divide-border">
          {EVENT_TYPE_PREF_OPTIONS.map((option) => {
            const pref = eventPrefByType[option.event_type]
            if (!pref) return null
            return (
              <div key={option.event_type} className="flex items-center justify-between py-3 gap-4">
                ...
                <button
                  ...
                  disabled={pauseAll}
                  onClick={() => toggleEventType(pref.id, !pref.enabled)}
                  className={`relative inline-flex h-6 w-11 shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-brand disabled:opacity-50 disabled:cursor-not-allowed ${
                    pref.enabled ? 'bg-brand' : 'bg-gray-200 dark:bg-gray-600'
                  }`}
                >
                  <span
                    className={`pointer-events-none inline-block h-5 w-5 rounded-full bg-white shadow ring-0 transition-transform duration-200 ${
        ...
        {pauseAll && (
          <p className="text-xs text-text-muted mt-3">
            Alert type toggles are disabled while all alerts are paused.
          </p>
        )}
      </div>
